...
#formulae =income ~.
formulae = income ~ employer_type + education + marital + occupation + relationship +race+ gender+ capitalgain + capitalloss + region + AgeGroups + WorkHour 
...
# best Mtry for a particular tree which we can use to figure out which mtry gives best result
...
t
# ============================================================================================
# Code for extracting rule from tree 
# For refernece we tried to extract rules using the same code we made in the class
# ============================================================================================
...
ruleExec <- unique(ruleExec)
ruleMetric <- getRuleMetric(ruleExec, X, census_test$income)
ruleMetric[1,]
# ============================================================================================
# Code finishes here
# ============================================================================================
...
#explain_forest(rf, interactions = TRUE, data = census_train)
...
#       census_test <- census[data1==2,]
#       
#       rf<- randomForest(formulae, data = census_train, ntree=numbtree[i], mtry=mtrys, importance =T)
#       confTr<-rf$confusion
#       accOOB =  c(accOOB,(confTr[1,1] + confTr[2,2]) / (confTr[1,1] + confTr[1,2] + confTr[2,1]+ confTr[2,2]))
#       
#       #- Accuracy = (TP+TN)/(TP+FP+FN+TN)
...
#     }
#     results[i,'ntree']<- numbtree[i]
#     results[i,'mtry']<- mtrys
#     results[i,'Acc OOB']<- mean(accOOB)
#     results[i,'Acc Test']<- mean(accTest)
#     results[i,'Precision']<- mean(precision)
#     results[i,'Recall']<- mean(recalls)
#     results[i,'Specificity']<- mean(specificity)
#     results[i,'F1 Score']<- mean(f1score)
#     #sd_Test = sd(accOOB1)
#     #sd_OOB = sd(accTest1)
#     #cat(i,".",j,": ntree = ",numbtree[i]," mtry = ",mtrys,", errors = ",max(accOOB),",",max(accTest),"\n")
#     
#     
#   }
# results
